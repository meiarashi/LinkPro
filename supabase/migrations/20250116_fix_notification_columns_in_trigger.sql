    ...
    IF pro_count > 0 THEN
      ...
      IF fifth_score IS NULL THEN
        ...
        INSERT INTO notifications (
          ...
          type,
          title,
          message,        -- contentからmessageに修正
          related_id,     -- related_project_idからrelated_idに修正
          related_type,   -- 新規追加
          is_read         -- read_statusからis_readに修正
        )
        SELECT 
          ms.ai_talent_id,
...
-- コメントを更新
COMMENT ON FUNCTION trigger_matching_calculation() IS 'プロジェクトが公開された時に自動でマッチングスコアを計算し、5位タイまでの人材に通知を送る（カラム名修正版）';