-- matching_scoresテーブルの制約を修正（すべての依存ビューを考慮）

-- 1. すべての依存ビューを確認して保存
CREATE TEMP TABLE IF NOT EXISTS temp_view_definitions AS
SELECT 
  viewname,
  definition
FROM pg_views
WHERE definition LIKE '%matching_scores%';

-- 2. 依存しているすべてのビューを削除
DROP VIEW IF EXISTS project_matching_candidates CASCADE;
DROP VIEW IF EXISTS ai_talent_profiles CASCADE;
DROP VIEW IF EXISTS matching_summary CASCADE;
DROP VIEW IF EXISTS top_matching_talents CASCADE;
...
-- 7. ビューを再作成（project_matching_candidates）
...
SELECT 
  ...
  p.rate_info,
  p.availability,
  (SELECT COUNT(*) FROM ai_use_cases WHERE user_id = p.id) as use_case_count,
  (SELECT COUNT(*) FROM ai_skills WHERE user_id = p.id) as skill_count,
