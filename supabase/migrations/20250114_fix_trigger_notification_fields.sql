-- トリガー内の通知作成部分を修正（実際のテーブル構造に合わせる）
...
BEGIN
  ...
  IF NEW.status = 'public' AND (OLD.status IS NULL OR OLD.status != 'public') THEN
    ...
    IF pro_count > 0 THEN
      ...
      -- 通知を作成（上位5名のみ） - 修正：実際のカラム名に合わせる
      INSERT INTO notifications (
        user_id,
        type,
        title,
        message,  -- contentではなくmessage
        related_id,  -- related_project_idではなくrelated_id
        related_type,
        is_read  -- read_statusではなくis_read
      )
      SELECT 
        ms.ai_talent_id,
