    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          {profile.profile_details?.ai_skills && profile.profile_details.ai_skills.length > 0 && (
            <div className="bg-white p-4 rounded-lg shadow-sm">
              <h2 className="text-base font-semibold text-gray-800 mb-3">AIスキル</h2>
              <div className="space-y-2">
                {profile.profile_details.ai_skills.map((skillKey) => {
                  const skill = AI_SKILLS[skillKey];
                  if (!skill) return null;
                  return (
                    <div key={skillKey} className="p-2 rounded border border-blue-200 bg-blue-50">
                      <div className="text-sm font-medium text-blue-900">{skill.label}</div>
              ...
              {profile.profile_details?.ai_tools && profile.profile_details.ai_tools.length > 0 && (
                <div className="mt-3">
                  <h3 className="text-xs font-medium text-gray-700 mb-2">使用ツール</h3>
                  <div className="flex flex-wrap gap-1">
                    {profile.profile_details.ai_tools.slice(0, 5).map((tool, index) => (
                      <span key={index} className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full text-xs">
                        {tool}
                      </span>
                    ))}
                    {profile.profile_details.ai_tools.length > 5 && (
                      <span className="text-xs text-gray-500">+{profile.profile_details.ai_tools.length - 5}</span>
                    )}
                  </div>
                </div>
              )}
